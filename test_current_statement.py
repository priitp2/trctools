...
class TestCurrentStatement(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.cstat = CurrentStatement(CURSOR, None)
        cls.happy_ops = {
    ...
    def test_add_opt(self):
        ...
        with self.assertRaisesRegex(BaseException, 'add_ops: got cursor *'):
            self.cstat.add_ops(wrong_cs)

        wrong_ops = Ops('STAR', WRONG_CURSOR, " ", FNAME, 3) 
        with self.assertRaisesRegex(BaseException, 'add_ops: unknown ops: *'):
