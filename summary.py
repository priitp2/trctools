...
class SummaryDuckdb:
    def __init__(self, dbdir):
        self.dbdir = dbdir
        d.sql("create or replace view elapsed_time as select sql_id, exec_id, last( ts order by ts) - first(ts order by ts) as ela from read_parquet('{}') where ts is not null group by sql_id, exec_id;".format(dbdir))

    def summary(self):
        res = d.sql("select sql_id, count(*) execs, median(ela), percentile_disc(0.99) within group(order by ela) from elapsed_time group by sql_id order by execs").fetchall()
        for r in res:
    ...
    def outliers(self, sql_id, thresold):
        res = d.sql("""select cursor_id "cursor", exec_id, ops, elapsed_time, ifnull(rows_processed, -1) "rows", ts, wait_name, file_name, line from read_parquet('{}') where sql_id = '{}' and exec_id in (select exec_id from elapsed_time where sql_id = '{}' and ela > {}) order by exec_id, ts""".format(self.dbdir, sql_id, sql_id, thresold))
        print(res)
    def waits(self, sql_id):
        res = d.sql("""select wait_name wait, count(*) count, sum(elapsed_time) sum, median(elapsed_time) median, percentile_disc(0.99) within group(order by elapsed_time) "99th percentile" from read_parquet('{}') where sql_id = '{}' and wait_name <> '' group by wait_name order by count(*) desc""".format(self.dbdir, sql_id))
        print(res)
#        for r in res:
#            print(r)
