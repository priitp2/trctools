...
class TestCallTracker(unittest.TestCase):
    ...
    def test_add_cursor(self):
        ...
        self.tracker.add_latest_cursor(test.CURSOR)
        # There is a special test.CURSOR '#0'
    ...
    def test_add_parse(self):
        ...
        self.tracker.add_ops(test.CURSOR, test.CORRECT_OPS['PARSE'])
        # add_parsing_in adds item to latest_test.CURSORs
        cs = self.tracker._get_cursor(test.CURSOR)
        ...
        # This merges the item in tr.latest_test.CURSORs with statements and overwrites the item
        self.tracker.add_ops(test.CURSOR, test.CORRECT_OPS['PARSE'])
        cs = self.tracker._get_cursor(test.CURSOR)
        self.assertNotEqual(cs, None)
    ...
    def test_add_fetch(self):
        ...
        cs = self.tracker.latest_cursors[test.CURSOR]
    ...
    def test_add_close(self):
        ...
        self.tracker.add_ops(test.CURSOR, test.CORRECT_OPS['CLOSE'])
    ...
    def test__get_cursor(self):
        ...
        self.tracker.add_pic(test.CURSOR, test.CORRECT_OPS['PIC'])
        self.assertEqual(self.tracker._get_cursor(test.CURSOR).cursor, test.CURSOR)
        self.assertEqual(self.tracker._get_cursor('#123'), None)
    ...
    def test_add_pic(self):
        ...
        # Another PIC should close existing test.CURSOR/interaction
        self.tracker.add_pic(test.CURSOR, test.CORRECT_OPS['PIC'])
        crs = self.tracker.latest_cursors[test.CURSOR]
    ...
    def test_add_ops_add_pic(self):
        ...
        self.tracker.add_ops(test.CURSOR, test.CORRECT_OPS['PIC'])
        self.assertEqual(len(self.tracker.latest_cursors), 2)
        self.assertEqual(len(self.tracker.db.batches), 2)
